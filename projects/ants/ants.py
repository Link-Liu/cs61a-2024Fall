...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not(search_location.is_hive) :
            flag += 1
            if  self.lower_bound <= flag <= self.upper_bound:
                if not random_bee(search_location.bees):
                    search_location = search_location.entrance
                else:
                    return random_bee(search_location.bees)
            else:
                search_location = search_location.entrance
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bee_in_place = list(self.place.bees)
        flag = self.health <= amount
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target is not None:
            # Store the initial duration of the slow effect for this bee
            self.bees_slowed[target] = 5
            target.slowed = True

            def slowed_action(gamestate):
                # Get the remaining duration for this bee
                duration = self.bees_slowed[target]
                if duration > 0:
                    if gamestate.time % 2 == 0 :  # Even turn
                        Bee.action(target, gamestate)  # Call the original action
                    # Odd turn: do nothing (take no action)
                    self.bees_slowed[target] -= 1  # Decrease duration after each turn
                else:
                    target.slowed = False
                    Bee.action(target, gamestate)  # Revert to original action
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if target is not None and target.had_scared == False:
            target.scare(2)
            target.had_scared = True
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place = self.place
        counter = 0
        while place.is_hive is not True:
            if place.ant is not None :
                if place.ant.is_container:
                    self.dist[place.ant] = counter
                    if place.ant.ant_contained:
                        if place.ant.ant_contained.name != 'Laser':
                            self.dist[place.ant.ant_contained] = counter
                else:
                    if place.ant.name != 'Laser':
                        self.dist[place.ant] = counter
            if place.bees != []:
                for bee in place.bees:
                    self.dist[bee] = counter
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        return max(2 - 0.25 * distance - self.insects_shot * 0.0625 ,0)
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.scared_turns_left = 0  # Track remaining scared turns for each bee
        self.slowed = False         # Track if the bee is slowed
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.scared_turns_left:
            self.move_backward()
            self.scared_turns_left -= 1
        else:
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        self.scared_turns_left = length

    def move_backward(self):
        """Move the bee one step backward if possible."""
        if not self.place.entrance.is_hive:  # Check if the bee is next to the Hive
