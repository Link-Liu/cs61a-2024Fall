...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    result = [paragraphs[index] for index in range(len(paragraphs))if select(paragraphs[index]) == True ]
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def check(container):
        lower = remove_punctuation(container.lower())
        list_word = lower.split()
        for i in subject:
            if i in list_word:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    num,right = len(typed_words) - len(source_words),0
    ...
    if len(source_words) != 0:
        for i in range(len(source_words)):
            if source_words[i] ==typed_words[i]:
                right += 1
        return right / len(typed_words) * 100 if num > 0 else right / (len(typed_words) + num) * 100
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        thing = (typed,source)
        if thing in cache:
            cache_result,cache_limit = cache[thing]
            if limit <= cache_limit: 
                return cache_result
        result = diff_function(typed, source, limit)
        cache[thing] = (result,limit)
        return result
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        if typed_word == word_list[i]:
            return word_list[i]
    list_diff = [diff_function(typed_word,word_list[i],limit) for i in range(len(word_list))]
    if min(list_diff) > limit:
        return typed_word
    for word in word_list:
        if diff_function(typed_word,word,limit) == min(list_diff):
            return word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    def check(typed,source,limit):
        ...
        if source == "":
            source += " " 
        if typed[0] == source[0]:
            return check(typed[1:],source[1:],limit) 
        if typed[0] != source[0]:
            return 1+check(typed[1:],source[1:],limit - 1) 
    return check(typed,source,limit)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def check(typed, source, user_id):
        num_right,total = 0,len(source)
        for index in range( len(typed)):
            if typed[index] == source[index]:
                num_right += 1
            else:
                return {'id': user_id, 'progress': num_right/total}
        return {'id': user_id, 'progress': num_right/total}
    d = check(typed, source, user_id)
    upload(d)
    return d['progress']
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_time in tpp:
        times += [[player_time[i+1]-player_time[i] for i in range(len(words))]]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    def check(word_indices,player_indices,times,index):
        container = [get_time(times,i,index) for i in player_indices]
        goal = min(container)
        for index2 in player_indices:
            if times[index2][index] == goal:
                return index2
    result = []
    for _ in player_indices:
        result += [[]]
    for index in word_indices:
        index2 = check(word_indices,player_indices,times,index)
        result[index2] += [words[index]]    
    return result
